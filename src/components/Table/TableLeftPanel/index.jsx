...
const TableLeftPanel = ({filter, setFilter}) => {
    return (
				...
				<ul>
					{btnsSelectStatus.map((btn) => {
						return <li key={btn.name}>
									<Link onClick={() => setFilter({...filter, status: btn.status})} data-value={btn.status} data-role="left-status" href="#" className={``}>{btn.name}
									</Link>
								</li>
					})}
				</ul>
